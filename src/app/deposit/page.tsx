...
import { Trash2, PlusCircle, Copy, Check, Phone } from 'lucide-react';
...
import {flat} from "eslint-config-next";
...
export default function Deposits() {
  ...
  const [pendingUssdCode, setPendingUssdCode] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (await submitTransaction(payload)){
  ...
  const getLastTransaction = async () => {
    ...
    try {
      ...
      const transactionLink = transaction.transaction_link;
      setTransactionLink(transactionLink?? null)
      setLoading(false);
    } catch (err) {
      console.error('Get last transaction error:', err);
      let errorMsg = 'Erreur lors de la récupération de la transaction';
      if (
        typeof err === 'object' &&
        err !== null &&
        'response' in err &&
  ...
  const handleFinalizeLastTransaction = async () => {
    ...
    setLoading(true);
    try {
      await api.post(`/yapson/finalize-transaction-user`,{reference:lastTransaction.reference});
      const ussdCode = lastTransaction.ussd_code;
      setIsLastTransactionModalOpen(false);
      setLastTransaction(null);
      setSuccess('Transaction finalisée avec succès!');
      setLoading(false);
      if (transactionLink) {
      ...
      let errorMsg = 'Échec de la finalisation de la transaction';
      if (
        typeof err === 'object' &&
        err !== null &&
        'response' in err &&
  ...
  const handleCancelLastTransaction = async () => {
    ...
    try {
      ...
      let errorMsg = "Échec de l'annulation de la transaction";
      if (
        typeof err === 'object' &&
        err !== null &&
        'response' in err &&
  ...
  const handleDepositModalConfirm = async () => {
    ...
    if (pendingPayload) {
      if (await submitTransaction(pendingPayload)){
        await getLastTransaction()
      }
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      ...
      {/* Transaction Details Modal */}
        ...
        {/* USSD Fallback Dialog */}
        {showUssdDialog && pendingUssdCode && (
          <div className="fixed inset-0 flex items-center justify-center z-[60] bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-sm mx-4">
              <div className="flex flex-col items-center text-center">
                <div className="w-14 h-14 rounded-full bg-orange-100 dark:bg-orange-950 flex items-center justify-center mb-4">
                  <svg className="w-7 h-7 text-orange-600 dark:text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                </div>
                <h3 className="text-lg font-semibold text-black dark:text-white mb-2">
                  {t("Composer le code USSD")}
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                  {t("Nous avons tenté d'ouvrir le composeur automatiquement. Si cela n'a pas fonctionné, composez manuellement le code ci-dessous :")}
                </p>
                <div className="w-full flex items-center justify-between p-3 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg mb-4">
                  <p className="text-sm sm:text-xl font-mono font-bold text-orange-600 dark:text-orange-400 tracking-widest flex-1 text-center">
                    {pendingUssdCode}
                  </p>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(pendingUssdCode);
                      setUssdCopied(true);
                      setTimeout(() => setUssdCopied(false), 2000);
                    }}
                    className="ml-2 p-1.5 rounded-md hover:bg-orange-100 dark:hover:bg-orange-900 transition-colors text-orange-600 dark:text-orange-400"
                    title={t("Copier")}
                  >
                    {ussdCopied ? <Check className="w-5 h-5 text-green-500" /> : <Copy className="w-5 h-5" />}
                  </button>
                </div>
                <div className="flex gap-3 w-full">
                  <a
                    href={`tel:${pendingUssdCode.replace(/#/g, '%23')}`}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-orange-600 text-orange-600 dark:text-orange-400 dark:border-orange-400 rounded-md text-center hover:bg-orange-50 dark:hover:bg-orange-950 transition-colors text-sm font-medium"
                  >
                    <Phone className="w-4 h-4" />
                    {t("Composer")}
                  </a>
                  <button
                    onClick={() => { setShowUssdDialog(false); setPendingUssdCode(null); router.push('/dashboard'); }}
        ...
        {isLastTransactionModalOpen && lastTransaction && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-30">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md mx-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-black dark:text-white">
                  {t("Dernière transaction")}
                </h3>
                <button
                  onClick={() => { setIsLastTransactionModalOpen(false); setLastTransaction(null); }}
                  className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                  ...
                  <span className="font-medium text-black dark:text-white">{lastTransaction.amount} FCFA</span>
                </div>
                ...
                <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                  ...
                  <span className={`font-medium ${
                    ...
                    {lastTransaction.status.charAt(0).toUpperCase() + lastTransaction.status.slice(1)}
                  </span>
                </div>

                <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                  <span className="text-gray-600 dark:text-gray-400">{t("Référence")}</span>
                  <span className="font-medium text-black dark:text-white">{lastTransaction.reference}</span>
                </div>

                <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                  <span className="text-gray-600 dark:text-gray-400">{t("Date")}</span>
                  <span className="font-medium text-black dark:text-white">
                    {new Date(lastTransaction.created_at).toLocaleString('fr-FR')}
                  </span>
                </div>
                ...
                {lastTransaction.phone_number && (
                  <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                    ...
                    <span className="font-medium text-black dark:text-white">{lastTransaction.phone_number}</span>
                  </div>
                )}

                {lastTransaction.network && (
                  <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                    <span className="text-gray-600 dark:text-gray-400">{t("Réseau")}</span>
                    <span className="font-medium text-black dark:text-white">
                      {lastTransaction.network.public_name || lastTransaction.network.name}
                    </span>
                  </div>
                )}
                ...
                {lastTransaction.app && (
                  <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                    <span className="text-gray-600 dark:text-gray-400">{t("Plateforme")}</span>
                    <span className="font-medium text-black dark:text-white">
                      {lastTransaction.app.public_name || lastTransaction.app.name}
                    </span>
                  </div>
                )}

                {lastTransaction.user_app_id && (
                  <div className="flex justify-between items-center py-2 border-b dark:border-gray-700">
                    <span className="text-gray-600 dark:text-gray-400">{t("Bet ID")}</span>
                    <span className="font-mono font-medium text-black dark:text-white">{lastTransaction.user_app_id}</span>
                  </div>
                )}

                {!transactionLink && lastTransaction.ussd_code && lastTransaction.ussd_code.trim() !== '' && (
                  <div className="mt-4 p-4 bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-lg">
                    <p className="text-sm font-medium text-orange-700 dark:text-orange-300 mb-2">{t("Code USSD à composer")}</p>
                    <p className="text-sm sm:text-xl font-mono font-bold text-orange-600 dark:text-orange-400 text-center tracking-widest break-all">
              ...
              <div className="flex justify-end space-x-3 mt-6">
                <button
                  ...
                  disabled={loading}
                  className="px-4 py-2 text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors disabled:opacity-50"
                >
                  {t("Annuler")}
                </button>
                <button
                ...
                >
                  {loading ? t('Traitement...') : t("Finaliser")}
                </button>
              </div>
            </div>
          </div>
      ...
      {/* <div className="mt-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-black dark:text-white">{t('Recent Deposits')}</h2>
        ...
        {/* <div className="p-6"> */}
          ...
          {/* <div className="space-y-4"> */}
            ...
            {/* <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <h3 className="text-lg font-medium text-black dark:text-white mb-2">{t('No recent deposits')}</h3>
      ...
      {/* <div className="mt-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-black dark:text-white">{t('Frequently Asked Questions')}</h2>
        ...
        <div className="p-6 space-y-4">
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <details className="group">
              <summary className="flex justify-between items-center p-4 cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 text-black dark:text-white">
